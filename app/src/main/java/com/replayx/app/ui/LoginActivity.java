...
public class LoginActivity extends AppCompatActivity {
    ...
    private void validateKey(String key, boolean isAutoLogin) {
        exec.execute(() -> {
            ...
            try {
                ...
                int minutes = 0;
                String user = "";
                // true só quando o Firestore tem um timestamp REAL gravado em firstUsed
                // (o painel cria a key com firstUsed = null, então fields.has("firstUsed")
                // sozinho não basta — precisa checar se tem timestampValue de fato)
                boolean hasRealFirstUsed = false;
                ...
                if (pf.length() > 0) {
                    try {
                        ...
                        // Se o PATCH falhar (ex: 403 por causa das Security Rules do Firestore),
                        // o firstUsed/deviceId nunca ficam salvos de verdade e a key parece
                        // "resetar" a cada login. Não bloqueia o login, mas avisa na tela.
                        if (patchCode < 200 || patchCode >= 300) {
                            String errBody = "";
                            try {
                                java.io.InputStream es = pc.getErrorStream();
                                if (es != null) {
                                    Scanner esc = new Scanner(es, "UTF-8");
                                    StringBuilder esb = new StringBuilder();
                                    while (esc.hasNextLine()) esb.append(esc.nextLine());
                                    esc.close();
                                    errBody = esb.toString();
                                }
                            } catch (Exception ignoredErr) {}
                            final int fPatchCode = patchCode;
                            main.post(() -> android.widget.Toast.makeText(
                                LoginActivity.this,
                                "[AVISO] Sync da key falhou (HTTP " + fPatchCode + "). Verifique as Security Rules da colecao 'keys' no Firestore.",
                                android.widget.Toast.LENGTH_LONG
                            ).show());
                        }
                        pc.disconnect();
                        // Se PATCH falhou, não bloqueia o login — segue para o sucesso mesmo assim
                    } catch (Exception patchEx) {
                        final String fPatchErr = patchEx.getMessage();
                        main.post(() -> android.widget.Toast.makeText(
                            LoginActivity.this,
                            "[AVISO] Sync da key falhou: " + fPatchErr,
                            android.widget.Toast.LENGTH_LONG
                        ).show());
                    }
                }
    ...
    private void goMain(String user, int days, int minutes, long first, String status, long paused) {
        getSharedPreferences(PREFS, MODE_PRIVATE).edit()
            .putString("kx_user", user)
            .putInt("kx_days", days)
            .putInt("kx_minutes", minutes)
            .putLong("kx_first", first)
            .putString("kx_status", status)
            .putLong("kx_paused", paused)
            .apply();
